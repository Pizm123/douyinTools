...
class AdbCommon:
    ...
    def call(self, device, func_name, *params):
        ...
        if func_name == "screen":
            device.screen()
        if func_name == "click_point":
            device.click_point(params[0], params[1])
        if func_name == "slide_by_point":
            device.slide_by_point(params[0], params[1], params[2])


# 设备类
class AdbDevice:
    def __init__(self, serial_num, stop_time):
        # 命令间隔时间
        self.sleepTime = stop_time + random.random() * 0.2
        # 设备序列号
    ...
    def slide_by_point(self, param, x1, y1):
        command = "shell input swipe " + x1 + " " + y1 + " " + \
                  x1 + " " + self.config[param]['y2'] + " " + self.config[param]['time']
        self.cmd(command)
        time.sleep(self.sleepTime)
